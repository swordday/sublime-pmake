...
class PremakeCommand(sublime_plugin.WindowCommand):
	...
	def _run_premake(self, args):
		...
		self.window.run_command("exec", {"cmd": ["pmake", "--file=" + self._get_premake_filepath()] + args})
	...
	def _generate_and_load_makefiles(self):
		...
		proc = subprocess.Popen(["pmake", "--file=" + self._get_premake_filepath(), "gmake"], startupinfo = startupInfo, stderr = subprocess.STDOUT, stdout = subprocess.PIPE, cwd = os.path.dirname(self._get_premake_filepath()))
		retVal = proc.wait()
		if retVal != 0:
